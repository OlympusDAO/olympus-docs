...
description: "The Yield Repurchase Facility (YRF) is an automated system to use the yield generated by the Protocol to buyback OHM from the market. In addition to using yield to buy OHM, it funds additional purchases by exchanging the OHM it receives from purchases for its backing in USDS. This increases the purchasing power of the facility and reduces OHM supply."
sidebar_label: "Yield Repurchase Facility"
---

# Yield Repurchase Facility

The Yield Repurchase Facility (YRF) is a live mechanism designed to use protocol yield to buy back OHM from the market. In addition to using yield to buy OHM, the mechanism can exchange purchased OHM for its backing in USDS, increasing the purchasing power of the facility and reducing OHM supply.

## Mechanism

YRF is implemented as a smart contract in the Olympus V3 system. It is integrated with the Heart contract and triggered by regular beats.

The mechanism is designed to pull earned yield from the Treasury on a weekly cadence. On the weekly reset the system:
...
OHM buybacks using yield generated by the protocol were originally approved by Olympus governance, and the Yield Repurchase Facility was developed to automate that mandate. Current budgets, cadence, market creation, and execution history are dynamic and should be verified from current contract or indexer data before citing specific live values.
